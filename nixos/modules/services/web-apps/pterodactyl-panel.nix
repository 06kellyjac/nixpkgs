  ...
  env =
    (lib.filterAttrs (n: v: v != null) {
      ...
      RECAPTCHA_SECRET_KEY =
        if cfg.recaptcha.secretKeyFile != null then "@RECAPTCHA_SECRET_KEY@" else cfg.recaptcha.secretKey;
      RECAPTCHA_WEBSITE_KEY =
        if cfg.recaptcha.websiteKeyFile != null then
          "@RECAPTCHA_WEBSITE_KEY@"
        else
          cfg.recaptcha.websiteKey;
