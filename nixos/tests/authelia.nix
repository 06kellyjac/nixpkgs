...
import ./make-test-python.nix ({ pkgs, ... }: {
  ...
  nodes = {
    ...
    authelia = { config, pkgs, lib, ... }: {
      ...
      systemd.services.simplehttp = {
        script = "cd ${pkgs.writeTextDir "index.html" "hello"} && ${pkgs.python3}/bin/python -m http.server 8000";
  ...
  testScript = ''
    ...
    authelia.wait_for_unit("authelia-main.service")
    ...
    with subtest("Check for authelia"):
      ...
      assert "Login - Authelia" in client.succeed("curl --insecure -sSf -H Host:auth.example.com https://authelia:443/")

    with subtest("Check contacting basic http server via traefik with https works"):
      assert "hello" in \
        client.succeed("curl --insecure -sSf -H Host:static.example.com https://authelia:443/")
    
    with subtest("Test traefik and authelia"):
      with subtest("No details fail"):
        client.fail("curl --insecure -sSf -H Host:static-basic-auth.example.com https://authelia:443/")
      with subtest("Incorrect details fail"):
        client.fail("curl --insecure -sSf -u 'bob:wordpass' -H Host:static-basic-auth.example.com https://authelia:443/")
        client.fail("curl --insecure -sSf -u 'alice:password' -H Host:static-basic-auth.example.com https://authelia:443/")
      with subtest("Correct details pass"):
        assert "hello" in \
          client.succeed("curl --insecure -sSf -u 'bob:password' -H Host:static-basic-auth.example.com https://authelia:443/")
  '';
})
